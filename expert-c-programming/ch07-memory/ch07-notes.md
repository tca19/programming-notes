...
#### Segmentation Fault
Generated by an exception in the MMU (manager of virtual memory). Usually
dereferencing a pointer with uninitialized/illegal value (like a `NULL`
pointer). It can also happens when :
1. writing into a read-only text segment
2. running out of stack/heap memory
3. accessing something after it has been freed
4. writing outside an array or malloc'ed block bounds
5. free a pointer twice, free a pointer still in use, free an invalid pointer

A segmentation fault is a symptom, not the cause of the error. The cause is
"*why is the pointer set with an invalid value ?*".

When compiling for debugging or optimization, the memory content is changed so
the problem moves or disappears, making it harder to find.

```C
printf("%s\n", p->name); /* dangerous, segmentation fault when name is NULL */

printf("%s\n", p->name ? p->name : "(null)"); /* safer, ternary op, 1 line */
```

If the debugger tells you the faulty function is a library routine, chances are
you called it with a bad pointer (make sure a pointer is not `NULL` before
passing it to functions).

If you're unlucky, a bad pointer will point inside the address space used by the
program, thus it will overwrite (corrupts) some data.
